...
const NewEventForm = ({closeDialog, refreshEvents}) => {
    ...
    useEffect(() => {
        // console.log('form - NewEventForm:\t' + JSON.stringify(form));
        // console.log('form.start.date - NewEventForm:\t' + form.start.date);
    ...
    const CreateEvent = event => {
        ...
        form.start.date = format(
            new Date(form.start.date),
            "yyyy-MM-dd'T'HH:mm:ss.SSS"
        );
        form.end.date = format(
            new Date(form.end.date),
            "yyyy-MM-dd'T'HH:mm:ss.SSS"
        );

        console.log("form - CreateEvent:\t" + JSON.stringify(form));
        fetch(`${server_url}/newEvent`, {
            method: "POST",
    ...
    // const onStartCalendarChange = (nextValue) => {
    //   console.log('calendar-onChange-onStartCalendarChange:\t' + nextValue);
    //   setCalendarStartDate(nextValue);
    // }
    // const onEndCalendarChange = (nextValue) => {
    //   console.log('calendar-onChange-onEndCalendarChange:\t' + nextValue);
    //   setCalendarEndDate(nextValue)
    // };

    const selectStartDate = value => {
        // console.log('onClickDay:selectStartDate:\t' + value);
        setForm({...form, start: {...form.start, date: value}});
        setCalendarStartDate(value);
    };
    const selectEndDate = value => {
        // console.log('onClickDay:selectEndDate:\t' + value);
    ...
    const submitStartDate = event => {
        ...
        // console.log('ok-submitStartDate-CalendarStartDate:\t' + CalendarStartDate)
        let formatted = format(CalendarStartDate, "EEE. MMM. d, y");
        // console.log('ok-submitStartDate-CalendarStartDate-formatted("EEE. MMM. d, y"):\t' + formatted)
        setDisplayStartDate(formatted);

        // if(form.start.date === null) {
        //   console.log('ok-submitStartDate-form start date is null, will set it to CalendarStartDate:\t');
        //   setForm({ ...form, start: { ...form.start, date: CalendarStartDate }});
        // }
    ...
    const submitEndDate = event => {
        ...
        // console.log('CalendarEndDate:\t' + CalendarStartDate)
    ...
    return (
        <div>
            ...
            <ActionsSection>
                ...
                <ButtonCreate onClick={ev => CreateEvent(ev)} disabled={buttonDisabled}>
                    {status === "idle" ? (
                        "Create event"
                    ) : status === "loading" ? (
                        <SmallLoadingIcon/>
